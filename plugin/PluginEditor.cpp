...
HyperFrameAudioProcessorEditor::HyperFrameAudioProcessorEditor(HyperFrameAudioProcessor& processor)
    ...
    auto launchBitInvaderImport = [this] {
        presetChooser_ = std::make_unique<juce::FileChooser>(
            "Import a BitInvader preset",
            lastFileChooserDirectory_,
            "*.xpf");
        juce::Component::SafePointer<HyperFrameAudioProcessorEditor> safeThis(this);
        presetChooser_->launchAsync(
            ...
            [safeThis](const juce::FileChooser& chooser) {
                ...
                editor->lastFileChooserDirectory_ = file.getParentDirectory();
                const auto imported = editor->processor_.importBitInvaderXpf(file);
                editor->syncMotionStepControls();
                editor->updateModeLabels();
                editor->waveformDisplay_.repaint();
                editor->gbRamStatusLabel_.setText(
                    imported ? "Imported BitInvader preset " + file.getFileName()
                             : "Could not parse BitInvader XPF",
                    juce::dontSendNotification);
            });
    };
    presetFileButton_.onClick = [this, launchPresetImport, launchPresetExport, launchSoundFontImport, launchLsdjImport, launchBitInvaderImport] {
        ...
        menu.addSeparator();
        menu.addItem(3, "Import BitInvader XPF...");
        menu.addItem(4, "Import SF2 Bank...");
        menu.addItem(5, "Import LSDJ Bank...");
        menu.addSeparator();
        menu.addItem(6, "Rename Preset...", isImportedPreset);
        juce::Component::SafePointer<HyperFrameAudioProcessorEditor> safeThis(this);
        menu.showMenuAsync(juce::PopupMenu::Options().withTargetComponent(&presetFileButton_),
            [safeThis, launchPresetImport, launchPresetExport, launchSoundFontImport, launchLsdjImport, launchBitInvaderImport, currentIdx](int result) {
                auto* editor = safeThis.getComponent();
                if (editor == nullptr) return;
                if (result == 1) {
                    launchPresetImport();
    ...
    auto launchBytebeatImport = [this] {
        auto* optionsWindow = new juce::AlertWindow(
            ...
            juce::AlertWindow::QuestionIcon,
            this);
        optionsWindow->addTextEditor("expression", "(t*5&t>>7)|(t*3&t>>10)", "Expression");
        ...
        optionsWindow->addComboBox("target",
            { "Use as Raw Stream", "Convert to Wave Frames" },
            "Target");
        if (auto* targetBox = optionsWindow->getComboBoxComponent("target")) {
            targetBox->setSelectedId(1, juce::dontSendNotification);
        }
    ...
    normalizeLiveToggle_.setButtonText("Normalize");
    normalizeLiveToggle_.setColour(juce::ToggleButton::tickColourId, accentColour());
    normalizeLiveToggle_.setColour(juce::ToggleButton::textColourId, juce::Colour::fromRGB(218, 224, 228));
    addAndMakeVisible(normalizeLiveToggle_);
    ...
    normalizeLiveAttachment_ = std::make_unique<ButtonAttachment>(parameters, kParamNormalize, normalizeLiveToggle_);
...
void HyperFrameAudioProcessorEditor::resized() {
    ...
    normalizeLiveToggle_.setBounds(actionRow.removeFromLeft(72).reduced(3, 3));
...
void HyperFrameAudioProcessorEditor::updateModeLabels() {
    ...
    smoothButton_.setEnabled(!isRaw);

...
void WaveTable::setActiveLength(std::size_t length) {
    activeLength_ = std::clamp(length, kMinActiveLength, kMaxPoints);
}

void WaveTable::setBitDepth(int bits) {
    const auto clampedBits = std::clamp(bits, kMinBitDepth, kMaxBitDepth);
    if (clampedBits == bitDepth_) {
        return;
    }

    bitDepth_ = clampedBits;
    quantize();
}

void WaveTable::setSampleFormat(SampleFormat format) {
    if (format == sampleFormat_) {
        return;
    }
...
void WaveTable::setPoint(std::size_t index, float value) {
    ...
    points_[index] = quantizeValue(value, bitDepth_, sampleFormat_);
}
...
float WaveTable::point(std::size_t index) const {
    return points_[sourceIndexForActivePoint(index, activeLength_)];
}
